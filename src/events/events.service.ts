...
export class EventsService {
    ...
    getEvent = async (id: number) =>
        await this.episodeRepository.findOne({ id })

    getEventWithBookmarks = async (id: number, user: User) => {
        const event = await this.getEvent(id)
        if (!event) throw new NotFoundException('Event not found')
        const userWithBookmark = await this.userRepository.getUserWithBookmarks(
            user
        )
        return {
            ...event,
            isLiked: !!userWithBookmark.bookmarks.find(
                (bookmark) => bookmark.id === event.id
            ),
        }
    }

    getAllEventsMod = async (eventFilterDto: EventFilterDto) =>
        this.episodeRepository.getAllEvents(eventFilterDto)

    addEvent = async (
        eventCreds: AddEventDto,
        user: User
    ): Promise<Episode> => {
        const { categories, mode } = eventCreds
        console.log(eventCreds, 'eventCredseventCreds')
        const getCategories = await this.categoryRepository.getAllCategories(
            categories
        )
        return this.episodeRepository.insertEpisode(
            eventCreds,
            user,
            getCategories
        )
    ...
    addMultipleEvents = async (eventCreds: AddEventDto[], user: User) => {
        ...
        const uniqueCategoriesEntity = await this.categoryRepository.getAllCategories(
            uniqueCategories
        )
    ...
    updateEvent = async (
        ...
        return this.episodeRepository.updateEpisode(
            eventCreds,
            id,
            getCategories
        )
    ...
    updateEventStatus = async (
        ...
        try {
            ...
            const result :any= await this.episodeRepository.update(
                { id },
                { status }
            )
            this.logger.log(
                `updateEventStatus: updated the status of event with info ${JSON.stringify(
                    event_info
                )}`
            )
            ...
            this.logger.error(
                `updateEventStatus: Error updating the status of event with info ${JSON.stringify(
                    event_info
                )}`,
                err.stack
            )
    ...
    getBookmarkedEvents = async (user: User): Promise<Episode[]> => {
        ...
        try {
            ...
            this.logger.error(
                `getBookmarkedEvents: user ${user.name} was unable to get bookmarked events`
            )
    ...
    addBookmarkedEvent = async (
        ...
        try {
            const myUser :any= await this.userRepository.findOne(id, {
                relations: ['bookmarks'],
            })
            ...
            if (myUser.bookmarks.find((bookmark) => bookmark.id === event_id)) {
                this.logger.log(
                    `addBookmarkedEvent: Event ${episode.name} was already bookmarked by ${myUser.name}`
                )
            ...
            console.log(myUser,"sdkfjskldfjsd")
            await myUser.save()
            return myUser
        } catch (e) {
            ...
            this.logger.error(
                `addBookmarkedEvent: User ${user.name} could not bookmark event with id ${event_id}`,
                e.stack
            )
            throw new InternalServerErrorException(e)
        }
    ...
    removeBookmarkedEvent = async (
        ...
        try {
            ...
            this.logger.log(
                `removeBookmarkedEvent: Event ${episode.name} was removed from bookmarked events by ${myUser.name}`
            )
            await myUser.save()
            return true
        } catch (e) {
            this.logger.error(
                `removeBookmarkedEvent: Removing bookmarked event by user ${user.name}`
            )
            throw new InternalServerErrorException(e)
        }
    }
